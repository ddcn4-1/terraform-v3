...
# This allows Velero to access S3 and EBS snapshots
module "velero_irsa_role" {
  source  = "terraform-aws-modules/iam/aws//modules/iam-role-for-service-accounts-eks"
  version = "~> 5.0"

  role_name = "${var.project_name}-velero"

  role_policy_arns = {
    policy = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:policy/VeleroBackupPolicy"
  }
